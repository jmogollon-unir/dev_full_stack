...
public class EmailService {
    ...
    public void sendOrderCreatedNotification(OrderCreatedEvent event) {
        try {
            ...
            helper.setSubject("Confirmación de Pedido - Relatos de Papel" + event.getBody().getOrderId());
            helper.setText(buildEmailContent(event), true);

            mailSender.send(mimeMessage);

            log.info("Notificación de correo enviada exitosamente para el pedido orderId={} a {}", event.getBody().getOrderId(), recipient);
        } catch (Exception e) {
            log.error("Error al enviar notificación de correo para el pedido orderId={}", event.getBody().getOrderId(), e);
        }
    }
